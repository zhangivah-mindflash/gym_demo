...
export const modelSettingDefinitions: ModelSettingDefinition[] = [
  ...
  },
  {
    ...
    label: "动作指导提示词",
    defaultValue:
      "若用户上传了动作视频，必须优先基于视频中的实际动作表现给出指导：指出看到的具体问题（身体姿态、发力顺序、动作路径、节奏与稳定性），再结合用户的文字描述定位重点，输出可执行的纠错步骤和下一次训练的改进重点。必要时给出更稳妥的替代动作。不要输出与视频无关的通用模板。safetyFlags 只在真的存在风险时才填，没有就返回空数组。",
    group: "提示词",
    helpText: "用于动作指导，结合视频与用户描述做针对性分析。",
    inputType: "textarea",
  },
  {
    id: "review-system",
