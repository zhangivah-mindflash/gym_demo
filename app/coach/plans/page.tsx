  ...
  return (
    <StaffLayout currentPath="/coach/plans" role="coach">
      <section className="staff-header">
        <div>
          <p className="eyebrow">计划列表</p>
          <h1>多会员计划筛选与详情</h1>
          <p>这里已经从单页聚合改成列表页，可按门店和关键词筛选，并进入每个会员的计划详情页。</p>
        </div>
      </section>
      ...
      <section className="panel">
        <div className="toolbar">
          ...
          <select className="toolbar-select" value={location} onChange={(event) => setLocation(event.target.value)}>
            ...
            {Array.from(new Set(members.map((item) => item.locationName))).map((name) => (
            ...
            pageItems.map((member) => (
              <a className="config-row" href={`/coach/plans/${member.id}`} key={member.id}>
                <div>
                  ...
                  <p>{member.locationName} · {member.goalLabel}</p>
                </div>
                <div className="config-meta">
                  <span>{member.coachName}</span>
                  <span className="badge-outline">v{member.planVersion}</span>
                </div>
              </a>
            ))
          ) : (
            <div className="empty-state">当前筛选条件下没有会员计划。</div>
          )}
        </div>
